...
const Appointment = () => {
  ...
  const handleAppointment = async (e) => {
    ...
    try {
      const response = await axios.post('https://myhealth-server-side.vercel.app/appointment', formData)
      ...
      <div className="w-full h-full flex items-center justify-center">
        <form className="bg-gray-200 p-12 mt-5 text-xl flex flex-col gap-3" onSubmit={handleAppointment}>
          <div className="py-2 flex items-center mt-3 pt-4">
            ...
            <input type="text" className="w-[530px] p-1 ml-4 rounded" name="patient_name" required />
          </div>
          <div className="flex items-center my-2">
            <h1>Date of Birth</h1>
            <input type="date" className="p-2 ml-4 rounded w-[550px]" name="dob" required />
          </div>
          ...
          <div className="my-2 flex items-center">
            <h1>Phone Number</h1>
            <input className="w-[500px] rounded p-1 ml-3" type="tel" name="phone" required />
          </div>
          <div className="my-2">
          ...
          <div className="">
            ...
            <input type="date" className="w-[500px] my-2 p-1" name="appointmentdate" required />
          </div>
          <div className="pb-4">
            ...
            <div className="my-2">
              {['7.00', '7.30', '8.00', '8.30'].map((time) => (
                <button
                  ...
                  className={`border-2 border-blue-500 py-1 px-3 m-1 rounded ${
                    appointmentTime === time ? 'bg-blue-500 text-white' : ''
                  }`}
                >
                  {time}
                </button>
