  ...
  const handleReportSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'https://myhealth-server-side.vercel.app/reportsubmit',
        { pname, pcode, doctorcode, doctorname, reportfile, pmail, date },
        {
          headers: {
  ...
  return (
    <div className="relative w-full flex justify-center h-full items-center text-xl ">
      ...
      <form className="relative ml-16 p-8 bg-slate-400 rounded shadow-lg" onSubmit={handleReportSubmit}>
        ...
        <button type="submit" className="bg-blue-800 mt-5 px-5 py-2 ml-4 text-white rounded-2xl hover:bg-blue-500">
          Submit
        </button>
